...
public class Game {
    ...
    void drawPlaying() {
        switch (gameState) {
            ...
            case ROLLING -> drawRolling();
            case ROLL_MENU -> drawRollMenu();
            case SELECTING_DICE -> drawSelectingDice();
            case SELECTING_CATEGORY -> drawSelectingCategory();
        }
    }
            ...
            case PLAYING:
                switch (gameState) {
                    ...
                    case ROLLING:
                        break; // ignore input
                    case ROLL_MENU:
                        handleRollMenuInput(input);
                        break;
                    case SELECTING_DICE:
                        handleSelectingDiceInput(input);
                        break;
                    case SELECTING_CATEGORY:
                        handleSelectingCategoryInput(input);
                        break;
                }
                break;
            case GAME_OVER:
        ...
        if (key != null) {
            switch (key) {
                case ARROW_LEFT:
                    ...
                    break;
                case ARROW_RIGHT:
                    selected = Math.min(selected + 1, Game.DICE_COUNT - 1);
                    break;
                case SPACE:
